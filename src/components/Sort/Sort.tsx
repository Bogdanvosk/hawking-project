...
type TSort = 'По умолчанию' | 'По цене' | 'По количеству комнат'

const Sort = () => {
	const sortTypes = ['По умолчанию', 'По цене', 'По количеству комнат']

	const [sortType, setSortType] = useState<TSort>('По умолчанию')
	...
	const handleTypeClick = (selectedItem: TSort) => {
	...
	return (
		<div className={styles.sortContainer}>
			<div className={styles.sortLabel} onClick={handleLabelClick}>
						...
						<g clipPath='url(#clip0_2831_2828)'>
							<g opacity={selectIsOpen ? '1' : '0.6'}>
								<path
									...
									fill={selectIsOpen ? '#664EF9' : '#7A7F86'}
								/>
								<path
			...
			<div
				...
				<ul>
					{sortTypes.map((item, idx) => {
						return (
							<li
								onClick={() => handleTypeClick(item as TSort)}
								key={`${item}_${idx}`}>
								{item}
							</li>
						)
